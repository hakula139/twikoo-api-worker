...
- Avoid semicolons in prose. Use a period or rewrite the sentence so the relationship stays explicit.

### TypeScript

- Worker entry exports a default object satisfying `ExportedHandler<Env>` (b2-worker pattern).
- `Env` is hand-written in `src/types.ts`. Optional secrets (`?`) let the smoke-test path run without them. Handlers that need a secret must validate it.
